...
export default function VendorsPage() {
  ...
  const pageParam = Math.max(1, Number.isFinite(Number(pageParamRaw)) ? Number(pageParamRaw) : 1);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        // Search
        if (qParam) {
          const escaped = qParam.replaceAll(",", " "); // defensive (Supabase .or string)
    ...
    <div className="space-y-5">
      ...
      <header className="rounded-3xl border bg-white p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600">
              {loading ? "Loading…" : `${total.toLocaleString()} vendor${total === 1 ? "" : "s"} found`}
              {qParam ? (
                <>
                  {" "}
                  for <span className="font-medium text-zinc-900">“{qParam}”</span>
                </>
              ) : null}
            </p>
          </div>
        ...
        <div className="mt-4">
          ...
          {/* Quick filter chips */}
          <div className={cn("mt-3 flex flex-wrap gap-2", showFilters ? "" : "")}>
          ...
          <div className={cn("mt-3", showFilters ? "" : "hidden sm:block")}>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {SORTS.map((s) => (
                <Pill key={s.key} active={sortParam === s.key} onClick={() => setSort(s.key)}>
                  {s.label}
                </Pill>
              ))}
            </div>
      ...
      {!loading && total > 0 ? (
        <div className="flex items-center justify-between rounded-3xl border bg-white p-4">
          <p className="text-sm text-zinc-600">
            Page <span className="font-medium text-zinc-900">{pageParam}</span> of{" "}
